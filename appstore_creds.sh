...
then
    [ "$is_debug" = "yes" ] && echo "jq could not be found, installing it now..."
    # Install Homebrew if it's not installed
    if ! command -v brew &> /dev/null
    then
        [ "$is_debug" = "yes" ] && echo "Installing Homebrew..."
...
[ "$is_debug" = "yes" ] && echo "APPLE_KEY_ID $APPLE_KEY_ID"
[ "$is_debug" = "yes" ] && echo "APPLE_ISSUER_ID $APPLE_ISSUER_ID"
[ "$is_debug" = "yes" ] && echo "APPLE_PRIVATE_KEY $APPLE_PRIVATE_KEY"